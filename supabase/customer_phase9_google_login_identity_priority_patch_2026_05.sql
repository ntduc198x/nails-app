-- Phase 9 Google/social customer identity priority
-- Priority order:
-- 1) existing customer_accounts by auth.user.id
-- 2) customers by normalized email
-- 3) customers by normalized phone
-- Only create a new customer when all 3 miss.
  ...
  if v_registration_mode = 'USER' then
    ...
    values (
      ...
      case
        ...
        when v_phone is not null then 'PHONE_MATCH'
        else 'APP_SIGNUP'
      end
    )
