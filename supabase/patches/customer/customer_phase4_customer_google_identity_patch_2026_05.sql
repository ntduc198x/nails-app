-- Phase 4 customer identity cleanup
-- 1) add customers.email
-- 2) backfill from auth/profiles/customer_accounts
-- 3) match Google/customer identity by email before name
-- 4) preserve user-edited customer names and stop duplicate customer creation
...
from (
  select distinct on (ca.customer_id)
    ca.customer_id,
    lower(nullif(trim(coalesce(au.email, p.email)), '')) as email
  ...
  order by ca.customer_id, ca.created_at asc
) src
...
create unique index if not exists idx_customers_org_email_unique
  ...
  where email is not null and merged_into_customer_id is null;
...
begin
  ...
  if v_registration_mode = 'USER' then
    ...
    values (new.id, v_customer_id, v_org_id, case when v_email is not null then 'EMAIL_MATCH' when v_phone is not null then 'PHONE_MATCH' else 'APP_SIGNUP' end)
...
begin
  ...
  select * into v_auth_user from auth.users where id = auth.uid() limit 1;
  if v_auth_user.id is null then
    raise exception 'AUTH_USER_NOT_FOUND';
  end if;

  select ca.org_id into v_org_id from public.customer_accounts ca where ca.user_id = auth.uid() limit 1;
  if v_org_id is null then
    v_workspace := public.ensure_default_workspace();
    v_org_id := coalesce((v_workspace ->> 'org_id')::uuid, '00000000-0000-0000-0000-000000000001'::uuid);
  end if;

  v_display_name := nullif(trim(coalesce(v_auth_user.raw_user_meta_data ->> 'display_name', v_auth_user.raw_user_meta_data ->> 'full_name', split_part(coalesce(v_auth_user.email, ''), '@', 1), 'Customer')), '');
  ...
  if v_customer_id is null then
    insert into public.customers (org_id, name, full_name, email, phone, source)
    values (v_org_id, coalesce(v_display_name, coalesce(v_auth_user.email, 'Customer')), coalesce(v_display_name, coalesce(v_auth_user.email, 'Customer')), v_email, v_phone, 'APP_SIGNUP')
    returning id into v_customer_id;
  else
    update public.customers
    set
  ...
  if v_existing_account_id is not null then
    ...
    set user_id = auth.uid(),
        ...
        org_id = v_org_id,
        linked_by = case when v_email is not null then 'EMAIL_MATCH' when v_phone is not null then 'PHONE_MATCH' else 'APP_SIGNUP' end
    where id = v_existing_account_id;
  else
    insert into public.customer_accounts (user_id, customer_id, org_id, linked_by)
    values (auth.uid(), v_customer_id, v_org_id, case when v_email is not null then 'EMAIL_MATCH' when v_phone is not null then 'PHONE_MATCH' else 'APP_SIGNUP' end);
  end if;
