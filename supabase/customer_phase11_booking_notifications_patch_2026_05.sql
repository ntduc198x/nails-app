-- Phase 11 booking notifications patch:
-- Create a customer inbox notification when a public booking request is created.
...
begin
  ...
  if v_customer_id is not null then
    ...
    values (
      coalesce(v_auth_user_id, (select ca.user_id from public.customer_accounts ca where ca.customer_id = v_customer_id and ca.org_id = v_org_id order by ca.created_at asc limit 1)),
      v_customer_id,
      v_org_id,
      'Yêu cầu đặt lịch đã được gửi',
      'Tiệm đã nhận yêu cầu ' || coalesce(nullif(trim(p_requested_service), ''), 'đặt lịch') || ' vào ' || to_char(v_start at time zone 'Asia/Ho_Chi_Minh', 'DD/MM/YYYY HH24:MI') || '.',
      'BOOKING',
      false,
      now()
    );
