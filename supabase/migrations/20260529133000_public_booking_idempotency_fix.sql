...
declare
  ...
  v_existing_row public.booking_requests;
  v_customer_id uuid;
...
begin
  ...
  if v_normalized_phone is not null then
    ...
    where br.org_id = v_org_id
      and br.branch_id = v_branch_id
      and br.customer_phone = v_normalized_phone
    ...
    if v_existing_row.id is not null then
      return jsonb_build_object(
        'booking_request_id', v_existing_row.id,
        'status', v_existing_row.status,
        'applied_offer_id', v_existing_row.applied_offer_id,
        'applied_offer_claim_id', v_existing_row.applied_offer_claim_id,
        'applied_offer_code', v_existing_row.applied_offer_code
      );
    end if;
  ...
  if v_customer_id is null then
    ...
    set
      ...
      global_note = case
        when p_note is null or btrim(p_note) = '' then global_note
        when global_note is null or btrim(global_note) = '' then p_note
        when position(p_note in global_note) > 0 then global_note
        else global_note || E'\n' || p_note
      end,
      notes = case
        when p_note is null or btrim(p_note) = '' then notes
        when notes is null or btrim(notes) = '' then p_note
        when position(p_note in notes) > 0 then notes
        else notes || E'\n' || p_note
      end,
