...
async function sendTelegramBookingMessageV2(payload: {
  ...
  if (!telegramBotToken || !telegramChatId) {
    throw new Error("Thiếu TELEGRAM_BOT_TOKEN hoặc TELEGRAM_BOOKING_CHAT_ID");
  }
...
export async function processTelegramBookingNotification(body: unknown) {
  ...
  if (!record?.id) {
    throw new Error("Missing booking record");
  }
  ...
  try {
    ...
    if (!bookingRow?.id) {
      throw new Error("Không đọc được booking request sau khi claim.");
    }
...
export async function buildTelegramBookingNotificationResponse(body: unknown) {
  try {
    const result = await processTelegramBookingNotification(body);
